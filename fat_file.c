...
static fat_file fat_file_init_empty(bool is_dir, char *filepath) {
...
fat_file fat_file_init_orphan_dir(char *name, fat_table table, u32 start_cluster) {
    ...
    orphan_dir->dentry = orphan_entry;
    build_filename(orphan_entry->base_name, orphan_entry->extension,
                   (char *)&(orphan_dir->name));
    orphan_dir->start_cluster = start_cluster; // Only FAT32
    orphan_dir->table = table;
    return orphan_dir;
}
...
ssize_t fat_file_pread(fat_file file, void *buf, size_t size, off_t offset,
    ...
    while (bytes_remaining > 0) { // There are still bytes to read
        ...
        if (!fat_table_cluster_is_valid(cluster)) {
            break;
        }
...
ssize_t fat_file_pwrite(fat_file file, const void *buf,
    ...
    off_t original_offset = offset, cluster_off = 0, starting_point = 0;
    ...
    while (bytes_remaining > 0) { // There are still bytes to write
        ...
        if (bytes_remaining > 0) {
            cluster = next_or_new_cluster(file, cluster);
            if (errno != 0 || !fat_table_cluster_is_valid(cluster)) {
                break;
            }
        }
    }
